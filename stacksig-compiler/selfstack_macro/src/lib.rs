...
fn impl_selfstack(input: TokenStream2) -> TokenStream2 {
    ...
    let final_type: Ident = if params.len() == 3 {
        ...
        parse2(TokenStream2::from_str("StackedSigma").unwrap()).unwrap()
    };

    let types = (0..height).map(|i| {
        let name = format_ident!("S{}", (1 << (i + 1)) as usize);
        if i == 0 {
            quote! {
                type #name = SelfStacker<#sigma>;
            }
        } else {
            let before = format_ident!("S{}", (1 << i) as usize);
            if i == height - 1 {
                quote! {
...
mod tests {
    ...
    fn impl_selfstack_works() {
        let tokens = TokenStream2::from_str("100235, Schnorr").unwrap();
