...
impl PartialBindingCommScheme for QBinding {
    ...
    fn gen<R: CryptoRngCore>(
        ...
        let (inner_ck, inner_ek) =
            if self.is_base() {
                ...
                let (inner_ck, inner_ek) = QBinding::new(
                    self.q - 1,
                )
                .gen(&pp.extract(()), inner_side, rng);

                (inner_ck.compose(), inner_ek.compose())
            };

        let outer_side = binding_index.get_outer();
        let (outer_ck, outer_ek) = HalfBinding.gen(
            pp.get_outer(),
            outer_side,
            rng,
        );
    ...
    fn equivcom<'a, M: Message + ?Sized>(
        ...
        let (inner_comm, inner_aux, outer_rand) = if self
            .is_base()
        {
            // Base case
            ...
            let message = match inner_side {
                Side::One => {
                    (msg[bound_index].clone(), mdefault)
                }
                Side::Two => {
                    (mdefault, msg[bound_index].clone())
                }
            };
            // Get inner commitment and auxiliary randomness
            let (inner_comm, inner_aux) = HalfBinding
                .equivcom(
                    pp.base_inner(),
                    ek.base_inner(),
                    &message,
            ...
            let (inner_comm, inner_aux) =
                QBinding::new(self.q - 1).equivcom(
                    &pp.extract(()),
                    &ek.extract(),
                    &message,
                    inner_rand,
    ...
    fn bind<'a, M: Message + ?Sized>(
        ...
        let (comm1, comm2) = if self.is_base() {
            ...
            let comm1 = HalfBinding.bind(
                pp,
                ck,
                &(msg[0].clone(), msg[1].clone()),
                r,
            );
            let comm2 = HalfBinding.bind(
                pp,
                ck,
                &(msg[2].clone(), msg[3].clone()),
                r,
            );
            ...
            let comm1 = inner_q.bind(
                pp,
                ck,
                &msg[0..self.inner_length()].to_vec(),
                r,
            );
            let comm2 = inner_q.bind(
                pp,
                ck,
                &msg[self.inner_length()..].to_vec(),
                r,
            );
    ...
    fn equiv<'a, M: Message + ?Sized>(
        ...
        let (
            ...
            let old_inner_message: Vec<Rc<M>> = (0..self
                .inner_length())
                .map(|i| {
            ...
            let outer_side = ek
                ...
                .get_outer();
            let chunk1 =
                new[0..self.inner_length()].to_vec();
            let chunk2 =
                new[self.inner_length()..].to_vec();
            let new_inner_message: &Vec<Rc<M>> =
                match outer_side {
                    Side::One => &chunk1,
                    Side::Two => &chunk2,
                };
            ...
            // Commit to every chunk with the new auxiliary
            // randomness
            let new_comm1 = inner_q.bind(
                pp,
                ck,
                &chunk1,
                &new_inner_aux,
            );
            let new_comm2 = inner_q.bind(
                pp,
                ck,
                &chunk2,
                &new_inner_aux,
...
mod tests {
    ...
    fn sandbox() {
        ...
        const B: usize = 2;
        const Q2: usize = 3;
        let (qbinding1, binding_index1) =
            QBinding::init(Q1, B);
        let (qbinding2, binding_index2) =
            QBinding::init(Q2, B);
        ...
        let aux1 =
            Randomness::random(&mut rng1.clone(), Q1);
        let aux2 = Randomness {
            inner: aux1.compose(),
            outer: halfbinding::Randomness::random(
                &mut rng2.clone(),
            ),
        };
        ...
        let (ck1, ek1) = qbinding1.gen(
            &pp1,
            binding_index1,
            &mut rng1.clone(),
        );
        let (ck2, ek2) = qbinding2.gen(
            &pp2,
            binding_index2,
            &mut rng2.clone(),
        );
        ...
        let (comm_equivcom1, aux_old1) = qbinding1
            .equivcom(&pp1, &ek1, &msg1, Some(aux1));
        let (comm_equivcom2, aux_old2) = qbinding2
            .equivcom(&pp2, &ek2, &msg2, Some(aux2));
        assert!(aux_old1 == aux_old2.extract(()));
        let aux_new1 = qbinding1.equiv(
            &pp1,
            &ek1,
            &msg1,
            &msg_equiv1,
        ...
        let comm_bind1 = qbinding1.bind(
            &pp1,
            &ck1,
            &msg_equiv1,
            &aux_new1,
        );
        let comm_bind2 = qbinding2.bind(
            &pp2,
            &ck2,
            &msg_equiv2,
            &aux_new2,
    ...
    fn test_qbinding_recursive_works() {
        ...
        let aux_new = qbinding
            .equiv(&pp, &ek, &msg, &msg_equiv, &aux_old);
        let comm_bind =
            qbinding.bind(&pp, &ck, &msg_equiv, &aux_new);
    ...
    fn test_qbinding_base_works() {
        ...
        let (ck, ek) =
            qbinding.gen(&pp, binding_index, rng);

        let (comm_equivcom, aux) = qbinding.equivcom(
            &pp,
            &ek,
            &msg,
            Some(aux.clone()),
    ...
    fn test_qbinding_base_fails_when_equiv_vs_bind_msg_differs(
        ...
        let aux_new = qbinding
            .equiv(&pp, &ek, &msg, &msg_equiv, &aux);
        let comm_bind =
            qbinding.bind(&pp, &ck, &msg, &aux_new);
    ...
    fn test_qbinding_fails_when_bound_msg_changes() {
        ...
        let (comm1, aux1) = qbinding.equivcom(
            &pp,
            &ek,
            &msg,
            Some(aux.clone()),
        );
        let (comm2, aux2) = qbinding.equivcom(
            &pp,
            &ek,
            &msg_equiv,
            Some(aux),
        ...
        let comm1_bind =
            qbinding.bind(&pp, &ck, &msg_equiv, &aux1);
        let comm2_bind =
            qbinding.bind(&pp, &ck, &msg, &aux2);
