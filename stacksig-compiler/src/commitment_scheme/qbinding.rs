...
impl QBinding {
    ...
    pub fn equivcom<M: Message + ?Sized>(
        ...
        let inner_vec = Self::inner(binding_index, msg, inner_side);
        // Commit to v_a
        ...
        let mut outer_vec: (Option<&Commitment>, Option<&Commitment>) =
            (None, None);
        match outer_side {
            Side::One => outer_vec.0 = Some(&inner_comm),
            Side::Two => outer_vec.1 = Some(&inner_comm),
        };
        // Commit to v_b
    ...
    /// Commit to a 4-tuple message which make up 2 chunks of 2-tuple messages
    /// We use the inner commitment scheme to commit to each of the 2 chunks
    /// and the outer commitment scheme to commit to the 2 inner commitments
    ///
    /// ## Parameters
    /// `pp`: Public parameters
    /// `ck`: Commitment key
    /// `msg`: Messages to commit to
    /// `r`: Randomness (auxiliary variables needed for commitment)
    pub fn bind<M: Message + ?Sized>(
        ...
        let comm1 = HalfBinding::bind(
            &pp.inner,
            ck.inner_ck,
            (Some(msg.0), Some(msg.1)),
            r.inner,
        );
        let comm2 = HalfBinding::bind(
            &pp.inner,
            ck.inner_ck,
            (Some(msg.2), Some(msg.3)),
            r.inner,
        );
        HalfBinding::bind(
            &pp.outer,
            ck.outer_ck,
            (Some(&comm1), Some(&comm2)),
            r.outer,
        )
    }
    ...
    pub fn equiv<M: Message + ?Sized>(
        ...
        let inner_vec = Self::inner(binding_index, old, inner_side);
        let inner_new = Self::inner_new(new, outer_side);
        let new_inner_aux =
            HalfBinding::equiv(inner_ek, inner_vec, inner_new, old_aux.inner);
        // Recompute outer vector from inner vector (which is old) but with new inner aux randomness
        ...
        let outer_vec = match outer_side {
            Side::One => (Some(&inner_comm), None),
            Side::Two => (None, Some(&inner_comm)),
        };
        // Commit to inner_new with new inner aux to get new outer aux
        ...
        let new_outer_aux = HalfBinding::equiv(
            outer_ek,
            outer_vec,
            (Some(&new_inner_comm1), Some(&new_inner_comm2)),
            old_aux.outer,
        );
        Randomness {
            inner: new_inner_aux,
