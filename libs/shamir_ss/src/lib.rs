...
impl Share {
    fn new(index: u8, share: [u8; 32]) -> Share {
        Share { index, share }
    }
}
...
impl ShamirSecretSharing {
    ...
    fn generate_shares(&self) -> Vec<Share> {
        let mut shares: Vec<Share> = Vec::new();
        for i in 1..=self.total_shares {
            let share = Share::new(
                i,
                Scalar::random(&mut ChaCha20Rng::from_entropy()).to_bytes(),
            );
            shares.push(share);
        }
        shares
    ...
    // fn complete_shares(&self, subset: Vec<Share>) -> Vec<Share> {}
}
...
impl SecretData {
    ...
    pub fn recover_secret(
        ...
        for byte_to_use in 0..rounds {
            ...
            match SecretData::full_lagrange(&xs, &fxs) {
                ...
                Some(resulting_poly) => {
                    mycoefficients.push(
                        String::from_utf8_lossy(&resulting_poly[..])
                            .to_string(),
                    );
    ...
    fn accumulate_share_bytes(
        ...
        for c in coefficient_bytes {
            accumulator = SecretData::gf256_add(
                accumulator,
    ...
    fn full_lagrange(xs: &[u8], fxs: &[u8]) -> Option<Vec<u8>> {
        ...
        for i in 0..len {
            ...
            for j in 0..len {
                ...
                match (first_term, second_term) {
                    (Some(a), Some(b)) => {
                        ...
                        this_polynomial = SecretData::multiply_polynomials(
                            &this_polynomial,
                            &this_term,
                        );
                    }
            ...
            returned_coefficients = SecretData::add_polynomials(
    ...
    fn gf256_mul(a: u8, b: u8) -> u8 {
        if a == 0 || b == 0 {
            ...
            GF256_EXP[((u16::from(GF256_LOG[a as usize])
                + u16::from(GF256_LOG[b as usize]))
                % 255) as usize]
        }
    }
...
mod tests {
    ...
    fn it_can_recover_secret() {
        ...
        let s3 =
            vec![3, 23, 176, 163, 177, 165, 218, 113, 163, 53, 7, 251, 196];

        let new_secret =
            SecretData::recover_secret(3, vec![s1, s2, s3]).unwrap();
